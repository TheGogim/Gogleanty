...
try {
    ...
    switch ($action) {
        ...
        case 'login':
            ...
            // Bloqueo de seguridad: Solo el administrador puede usar la aplicación móvil
            if ($user['role'] !== 'admin') {
                recordLoginAttemptMobile($db, $ip, false); // Opcional, pero bueno para evitar intentos extraños
                sendJson([
                    'success' => false, 
                    'message' => 'La App Móvil es exclusiva para el administrador. Para ver tus álbumes compartidos, por favor ingresa usando nuestro sitio Web.'
                ], 403);
            }
            ...
            // Generar token
            // Usamos 16 bytes para generar un token hexadecimal de 32 caracteres.
            // Esto evita truncamientos silenciosos si la columna 'id' en 'sessions' es VARCHAR(32) o VARCHAR(64).
        ...
        case 'check':
            ...
            if ($row['role'] !== 'admin') {
                sendJson(['success' => false, 'valid' => false, 'message' => 'Acceso denegado: La app móvil es exclusiva para el administrador'], 403);
            }
