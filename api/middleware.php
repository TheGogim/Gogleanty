...
function requireAuthMobile()
{
    ...
    if (preg_match('/^Bearer\s+(.+)$/i', $authHeader, $matches)) {
        ...
        $stmt = $db->prepare("SELECT s.user_id FROM sessions s WHERE s.id = ?");
        $stmt->bind_param("s", $token);
        $stmt->execute();
        ...
        // Restricción colosal para forzar admin-only también a nivel de middleware
        if ($row['role'] !== 'admin') {
            header('Content-Type: application/json');
            http_response_code(403);
            echo json_encode([
                'success' => false,
                'message' => 'Acceso denegado: La app móvil es exclusiva para el administrador.'
            ]);
            exit;
        }
