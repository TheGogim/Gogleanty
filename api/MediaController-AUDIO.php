...
class MediaController
{
    ...
    public function uploadMedia($file)
    {
        ...
        try {
            $metadata = $this->extractMetadata($absolute_upload_path, $file_type, $mime_type);
            ...
            $metadata = [
                ...
                'height' => null,
                'duration' => null,
                'date_taken' => date('Y-m-d H:i:s', filemtime($absolute_upload_path)),
                'camera_make' => null,
                'camera_model' => null,
                'focal_length' => null,
                'aperture' => null,
        ...
        $stmt = $this->db->prepare("
            INSERT INTO media (
                ...
                gps_altitude, location_name
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");

        $stmt->bind_param(
            "ssssssiiiisssssssddds",
            $filename,
            $file['name'],
    ...
    public function getAllMedia($page = 1, $limit = ITEMS_PER_PAGE, $type = null, $favorite = null, $year = null, $month = null)
    {
        ...
        $count_sql = "SELECT COUNT(*) as total FROM media $where_clause";
        ...
        $sql = "SELECT * FROM media $where_clause ORDER BY date_taken DESC, date_uploaded DESC LIMIT ? OFFSET ?";
     ...
     */
    private function extractMetadata($file_path, $file_type, $mime_type)
    {
        $metadata = [
        ...
        if ($file_type === 'image') {
            ...
            if (function_exists('exif_read_data') && in_array($mime_type, ['image/jpeg', 'image/jpg'])) {
            ...
            if ($ffmpeg_metadata) {
                ...
                error_log("FFmpeg no disponible, usando fecha de archivo");
            }
        } elseif ($file_type === 'audio') {
            // AUDIOS: Extraer duración con FFmpeg
            $ffmpeg_metadata = $this->extractFFmpegMetadata($file_path);
            ...
            // Fecha de subida (no metadatos) - SOLO PARA AUDIOS
            $metadata['date_taken'] = date('Y-m-d H:i:s');
        }

        // Si no hay fecha, usar fecha del archivo
